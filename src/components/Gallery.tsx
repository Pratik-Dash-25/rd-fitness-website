...
export default function Gallery() {
  ...
  const categories = [
    ...
    { label: 'Equipment & Dumbbells', value: 'equipment' },
    { label: 'Facilities & AC Floor', value: 'facilities' },
  ...
  return (
    <section id="gallery" className="py-20 sm:py-28 bg-neutral-950">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredImages.map((img) => (
            <div
              ...
              className="group relative h-64 sm:h-72 rounded-2xl overflow-hidden bg-neutral-900 border border-neutral-800 cursor-pointer"
            >
              <img
                src={img.imageUrl}
                alt={img.title}
              ...
              <div className="absolute bottom-0 inset-x-0 p-5 flex items-end justify-between">
                <div>
                  <h3 className="font-display text-lg font-bold uppercase text-white tracking-wide mb-1">
                    {img.title}
                  </h3>
                  <p className="text-xs text-neutral-300 leading-tight line-clamp-1">
                    {img.caption}
                  </p>
                </div>
                <div className="p-2 rounded-lg bg-neutral-900/80 backdrop-blur-sm text-neutral-300 group-hover:text-rose-400 transition-colors">
